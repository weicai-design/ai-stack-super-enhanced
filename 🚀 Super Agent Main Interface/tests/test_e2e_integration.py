...
class TestE2EUserAuthentication(unittest.TestCase):
    ...
    def test_complete_user_workflow(self):
        ...
        # 注册用户
        registration_result = self.user_manager.register_user(user_data)
        self.assertTrue(registration_result["success"])
        
        # 用户登录
        login_result = self.user_manager.login_user(
        ...
        session_result = self.user_manager.create_session(session_data)
        self.assertTrue(session_result["success"])
        
        # 验证性能监控
        performance_stats = self.performance_monitor.get_performance_stats()
        self.assertIsNotNone(performance_stats)
        self.assertGreater(performance_stats["total_requests"], 0)
        
        # 验证数据持久化
        user_record = self.data_manager.get_user_by_id(login_result["user_id"])
        self.assertIsNotNone(user_record)
        self.assertEqual(user_record["username"], user_data["username"])
    ...
    def test_performance_benchmark(self):
        ...
        # 性能要求：10个用户注册应在5秒内完成
        self.assertLess(duration, 5.0, f"性能测试失败：10个用户注册耗时{duration:.2f}秒")
    ...
    def test_error_handling_scenarios(self):
        ...
        # 第一次注册
        result1 = self.user_manager.register_user(user_data)
        self.assertTrue(result1["success"])
        
        # 第二次注册（应失败）
        result2 = self.user_manager.register_user(user_data)
        self.assertFalse(result2["success"])
        self.assertEqual(result2["error"], "用户已存在")
        
        # 测试无效登录
        invalid_login = self.user_manager.login_user(
            username="nonexistent_user",
            password="wrongpassword"
        )
        self.assertFalse(invalid_login["success"])
        self.assertEqual(invalid_login["error"], "用户名或密码错误")
    ...
    def test_integration_scenarios(self):
        ...
        reg_result = self.user_manager.register_user(user_data)
        ...
        # 3. 验证用户会话数量
        user_sessions = self.user_manager.get_user_sessions(login_result["user_id"])
        self.assertEqual(len(user_sessions), 3)
        
        # 4. 验证性能数据
        stats = self.performance_monitor.get_performance_stats()
        self.assertGreaterEqual(stats["successful_operations"], 5)  # 注册+登录+3个会话
