...
class Filter:
    ...
    async def inlet(
        ...
        if self.valves.enable_expert_analysis and detected_system:
            expert_advice = await self.get_expert_analysis(detected_system, user_message, system_data)
    ...
    async def outlet(
        ...
        if self.valves.enable_auto_rag_ingest and user_message and ai_response:
            await self.ingest_interaction_to_rag(user_message, ai_response, user_id, validation_note)
        
        if self.valves.enable_interaction_learning:
            await self.submit_to_learning(user_message, ai_response, user_id, validation_note)
    ...
    async def query_erp(self, message: str, params: dict = None) -> Optional[str]:
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.get(f"{self.valves.erp_api}/api/finance/summary", timeout=5.0)
                if response.status_code == 200:
                    data = response.json()
    ...
    async def query_stock(self, message: str, params: dict = None) -> Optional[str]:
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.get(f"{self.valves.stock_api}/api/stock/price/{code}", timeout=5.0)
                if response.status_code == 200:
                    data = response.json()
    ...
    async def get_expert_analysis(self, system: str, user_question: str, system_data: Optional[str]) -> Optional[str]:
        """获取专家分析建议"""
        try:
            templates = {
    ...
    async def validate_with_rag(self, user_question: str, ai_response: str, user_id: str) -> Optional[str]:
...
"""
    ...
    async def ingest_interaction_to_rag(self, user_msg: str, ai_response: str, user_id: str, validation_note: str = None) -> bool:
        """需求4: 自动将对话和经验入库到RAG"""
...
"""
    ...
    async def submit_to_learning(self, user_msg: str, ai_response: str, user_id: str, validation_note: str = None) -> bool:
        """需求4: 提交到自我学习系统"""
    ...
    async def trigger_self_evolution(self, user_msg: str, ai_response: str) -> bool:
        """需求4: 触发自我进化"""
