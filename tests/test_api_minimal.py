...
def test_readyz():
    ...
    assert j["st_model"] is True
    assert j["vector_index"]["dimension"] == 384

def test_ingest_text_and_search():
    _clear_files()
    client.delete("/index/clear?remove_file=true&clear_kg=true")
    r = client.post("/rag/ingest", json={"text": "hello email test@example.com", "save_index": True})
    assert r.status_code == 200
...
def test_persistence_reload():
    r = client.post("/index/save"); assert r.status_code == 200
    r = client.post("/index/load"); assert r.status_code == 200
    r = client.get("/rag/search", params={"query": "email", "top_k": 3})
    assert r.status_code == 200
